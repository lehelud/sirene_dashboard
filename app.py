...
Fix: lecture parquet via pyarrow direct (arrow_extensions_enabled=False)
URL correcte : StockUniteLegale_utf8.parquet
"""
...
SECTEURS_CIBLES = ["Agriculture","Industrie","Construction","Commerce / Transport / HCR","Services aux entreprises"]
COLOR_MAP = {
...
@st.cache_data(ttl=86_400, show_spinner="Telechargement donnees SIRENE (~3 min)...")
def charger_donnees():
    st.write("Telechargement unites legales...")
    r = requests.get(URL_UL, timeout=300)
    r.raise_for_status()
    # Fix pyarrow ArrowInvalid : desactiver arrow_extensions
    ...
    df = tbl.to_pandas()
    st.write("Traitement des donnees...")
    # Filtrer secteur marchand (exclure cat juridique 7xxx et 9xxx)
...
def agreger_creations(df):
    now = datetime.now().year
    d = df[df["annee"].between(max(2010, now-10), now)]
    return d.groupby(["annee","grand_secteur","categorieJuridiqueUniteLegale",
        "trancheEffectifsUniteLegale"],dropna=False).size().reset_index(name="nb_creations").rename(
...
def agreger_cessations(df):
    ...
    return d.groupby(["annee_mois_cess","grand_secteur"]).size().reset_index(
        name="nb_cessations").rename(columns={"annee_mois_cess":"annee_mois"})


def agreger_naf(df):
    return df[df["etatAdministratifUniteLegale"]=="A"].groupby(
        ["section_naf","grand_secteur"]).size().reset_index(name="nb_entreprises")
...
def afficher_sidebar(df_c):
    st.sidebar.title("Filtres")
    annees = sorted(df_c["annee"].dropna().astype(int).unique().tolist())
...
def main():
    ...
    with st.spinner("Chargement des donnees SIRENE (~3 min au premier lancement)..."):
        df = charger_donnees()
    ...
    with tab2:
        d = df_cess[df_cess["grand_secteur"].isin(filtres["secteurs"])]\
            .groupby(["annee_mois","grand_secteur"])["nb_cessations"].sum().reset_index()
        d = d.sort_values("annee_mois")
        d = d[d["annee_mois"].isin(sorted(d["annee_mois"].unique())[-60:])]
        fig = px.line(d,x="annee_mois",y="nb_cessations",color="grand_secteur",
            color_discrete_map=COLOR_MAP,title="Cessations mensuelles",
    ...
    with tab3:
        d = df_naf[df_naf["grand_secteur"].isin(filtres["secteurs"])]\
    ...
    with tab4:
        ...
        d = df_taille[df_taille["grand_secteur"].isin(filtres["secteurs"])]\
            .groupby("libelle_taille")["nb_entreprises"].sum().reset_index()
        ...
        fig = px.bar(d,x="libelle_taille",y="nb_entreprises",color_discrete_sequence=["#3498db"],
            title="Repartition par taille d'effectifs",
