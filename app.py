...
# CSS - construit par concatenation pour eviter tout probleme de parsing
_CSS_LINES = [
    ...
    "[data-testid=\"stMetricLabel\"] { color:#546e7a !important; font-size:.72rem !important; font-weight:600 !important; text-transform:uppercase; letter-spacing:.4px; }",
    ...
    "[data-testid=\"stSidebar\"] .stRadio label[data-baseweb=\"radio\"] { border-radius:7px; padding:9px 12px !important; font-size:.88rem !important; font-weight:500 !important; color:#94a3b8 !important; transition:all .15s; }",
...
def page_vue_ensemble(data, sects):
    ...
    c12m=cm[cm["mois"].isin(mois_rec)&cm["grand_secteur"].isin(sects)]["nb_creations"].sum()
    ay=int(cm["annee"].max())
    an=cm[(cm["annee"]==ay)&cm["grand_secteur"].isin(sects)]["nb_creations"].sum()
    an1=cm[(cm["annee"]==ay-1)&cm["grand_secteur"].isin(sects)]["nb_creations"].sum()
    delta=(an-an1)/an1*100 if an1 else 0
    c1,c2,c3,c4,c5=st.columns(5)
    c1.metric("Entreprises actives",fmt(total)); c2.metric("Creations 12 mois",fmt(c12m))
    c3.metric(f"Creations {ay}",fmt(an),f"{delta:+.1f}% vs {ay-1}")
    c4.metric("Employeurs",fmt(nb_emp),f"{nb_emp/total*100:.1f}%"); c5.metric("ESS",fmt(nb_ess))
    st.divider()
    ...
    fig.update_traces(texttemplate="<b>%{label}</b><br>%{value:,.0f}  (%{customdata[0]:.1f}%)",textfont_size=12,marker_line_width=1.5,marker_line_color="#fff")
...
def page_tendances(data, sects):
    ...
    T(fig,360); fig.update_layout(title="Creations mensuelles - MM3",hovermode="x unified")
    st.plotly_chart(fig,use_container_width=True)
    st.divider()
    ...
    with col1:
        ...
        fig2=px.bar(sais,x="mois_nom",y="ecart",color=sais["ecart"].apply(lambda x:"+" if x>=0 else "-"),
            color_discrete_map={"+":"#27ae60","-":"#e74c3c"},title="Ecart a la moyenne (%)",labels={"mois_nom":"","ecart":"Ecart (%)"})
...
def page_structure(data, sects):
    ...
    with col1:
        ...
        fig=px.pie(fj_g,values="nb",names="forme_jur",hole=0.45,color_discrete_sequence=["#1565c0","#42a5f5","#90caf9","#bbdefb","#e3f2fd"])
    ...
    fig3.update_layout(yaxis=dict(title="Nb",showgrid=False),yaxis2=dict(title="Cumul %",overlaying="y",side="right",range=[0,110],showgrid=False),
        plot_bgcolor="rgba(0,0,0,0)",paper_bgcolor="rgba(0,0,0,0)",height=320,margin=dict(t=20,b=20,l=8,r=8),
        legend=dict(orientation="h",yanchor="bottom",y=1.02),xaxis=dict(tickangle=15,showgrid=False))
...
def page_survie(data):
    ...
    with col1:
        ...
        fig=px.imshow(pv,color_continuous_scale="RdYlGn",labels={"color":"Survie %","x":"Duree (ans)","y":"Annee"},zmin=0,zmax=100)
    ...
    if not s3.empty:
        fig3=px.line(s3,x="annee_creation",y="taux_survie",markers=True,color_discrete_sequence=["#1565c0"],
            labels={"annee_creation":"Annee de creation","taux_survie":"Survie %"})
        fig3.add_hline(y=s3["taux_survie"].mean(),line_dash="dash",line_color="#e74c3c",annotation_text=f"Moy. {s3['taux_survie'].mean():.1f}%")
        T(fig3,280); st.plotly_chart(fig3,use_container_width=True)

def page_carte(data, sects):
    ph("Carte","Repartition geographique")
    ...
    fig.add_scattermapbox(lat=md["lat"],lon=md["lon"],mode="markers",
        marker=go.scattermapbox.Marker(size=md["val"].apply(lambda v:max(7,min(38,v/vmax*38)) if vmax else 8),
            color=md["val"],colorscale="Blues",showscale=True,colorbar=dict(title=titre,thickness=10),opacity=0.82),
...
def main():
    ...
    sects=st.sidebar.multiselect("Secteurs",all_s,default=[s for s in all_s if s!="Autres"],label_visibility="collapsed")
