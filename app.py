...
CSS = (
    ...
    ".block-container { padding:1.5rem 2rem 3rem !important; background:#f4f6f8 !important; }"
    "[data-testid=\"metric-container\"] { background:#ffffff !important; border-radius:8px !important; padding:16px 20px !important; box-shadow:0 1px 4px rgba(0,0,0,.09) !important; border-left:4px solid #1565c0 !important; }"
    "[data-testid=\"stMetricLabel\"] { color:#4b5563 !important; font-size:.75rem !important; font-weight:600 !important; text-transform:uppercase; letter-spacing:.5px; }"
    "[data-testid=\"stMetricValue\"] { color:#1565c0 !important; font-size:1.55rem !important; font-weight:700 !important; }"
    ...
    "[data-testid=\"stSidebar\"] .stRadio label[data-baseweb=\"radio\"][aria-checked=\"true\"] { background:rgba(21,101,192,.4) !important; color:#ffffff !important; border-left:3px solid #42a5f5 !important; }"
    "[data-testid=\"stSidebar\"] .stRadio input[type=\"radio\"] { display:none !important; }"
    "[data-testid=\"stSidebar\"] [data-baseweb=\"tag\"] { background:#1565c0 !important; }"
    "h1, h2, h3, h4 { color:#111827 !important; }"
    "h2[data-testid=\"stHeading\"] { color:#111827 !important; font-weight:700 !important; border-bottom:2px solid #e5e7eb; padding-bottom:8px; margin-bottom:12px !important; }"
    "h3[data-testid=\"stHeading\"] { color:#1f2937 !important; font-weight:600 !important; margin-bottom:6px !important; }"
    ...
    ".stAlert p { color:#1e3a5f !important; font-size:.9rem !important; }"
    "[data-testid=\"stCaptionContainer\"] p { color:#6b7280 !important; font-size:.8rem !important; }"
...
def T(fig, h=380):
    fig.update_layout(
        ...
        legend=dict(orientation="h", yanchor="bottom", y=1.02, font_size=11,
                    bgcolor="rgba(255,255,255,0.8)", font_color="#374151"),
        xaxis=dict(showgrid=False, linecolor="#e5e7eb", tickfont=dict(color="#374151")),
        yaxis=dict(showgrid=True, gridcolor="#f3f4f6", linecolor="#e5e7eb",
                   tickfont=dict(color="#374151")),
        hoverlabel=dict(bgcolor="#111827", font_color="#fff", font_size=12)
    )
    return fig
...
def page_vue_ensemble(data, sects):
    section("Vue d'ensemble", "Source : INSEE / Base SIRENE - Secteur marchand francais")
    ...
    c1.metric("Entreprises actives au registre", fmt(total))
    c2.metric(f"Creations {ay} (annee complete)", fmt(an), f"{delta:+.1f}% vs {ay1}")
    c3.metric(f"Creations {annee_courante} (cumul, annee en cours)", fmt(an_cur))
    c4.metric("Dont employeurs (effectif connu)", fmt(nb_emp), f"{nb_emp/total*100:.1f}% des actives")
    st.info("**ESS (Economie Sociale et Solidaire)** : selon la liste officielle ESS France / data.gouv.fr, environ **1,1 million d'organisations** relevent de l'ESS en France (cooperatives, mutuelles, associations, fondations). Ce chiffre inclut les associations non employeuses. La variable ESS dans SIRENE est sous-renseignee et non representative.")
    st.divider()
    st.subheader("Repartition des entreprises actives par grand secteur")
    st.caption("Chaque rectangle est proportionnel au nombre d'entreprises actives dans ce secteur.")
    ...
    st.caption("Pour chaque secteur : quelle part des entreprises a des salaries ? Code NN = non-employeur OU entreprise recente (< 3 ans) dont le statut n'est pas encore connu de l'INSEE.")
    ...
    fig2.update_layout(xaxis=dict(range=[0,100], ticksuffix="%", tickfont=dict(color="#374151")),
...
def page_vue_ensemble(data, sects):
    ...
    c4.metric("Dont employeurs (effectif connu)", fmt(nb_emp), f"{nb_emp/total*100:.1f}% des actives")
    if nb_ess > 0:
        st.info(f"Parmi les {fmt(total)} entreprises actives au registre, **{fmt(nb_ess)} ({pct_ess:.1f}%)** relevent de l'**Economie Sociale et Solidaire (ESS)** (cooperatives, mutuelles, associations employeuses, fondations). Il s'agit du stock actuel d'entreprises, pas du nombre de creations.")
    st.divider()
    st.subheader("Repartition des entreprises actives par grand secteur")
    st.caption("Chaque rectangle est proportionnel au nombre d'entreprises actives dans ce secteur.")
    d = sa_f.groupby("grand_secteur")["nb"].sum().reset_index()
    ...
    st.divider()
    st.subheader("Quel est le statut employeur des entreprises ?")
    st.caption("Pour chaque secteur : quelle part des entreprises a effectivement des salaries ? Le code NN regroupe les non-employeurs ET les entreprises recentes (< 3 ans) dont le statut n'est pas encore connu de l'INSEE.")
    emp_f = emp[emp["grand_secteur"].isin(sects)].copy()
    ordre_emp = ["Employeur","Employeur occasionnel","Non-employeur / NC","Non renseigne"]
    tot_s = emp_f.groupby("grand_secteur")["nb"].sum().rename("tot")
    emp_pct = emp_f.merge(tot_s, on="grand_secteur")
...
def page_tendances(data, sects):
    ...
    T(fig, 360); fig.update_layout(title="Creations mensuelles (trait epais = moyenne mobile 3 mois pour lisser les variations)", hovermode="x unified")
    st.plotly_chart(fig, use_container_width=True)
    ...
    with col1:
        ...
        fig2 = px.bar(sais, x="mois_nom", y="ecart",
            ...
            color_discrete_map={"Au-dessus": "#16a085", "En-dessous": "#e74c3c"},
    ...
    with col2:
        ...
        st.caption("Taux de croissance annuel des creations. Attention : l'annee en cours est partielle.")
...
def page_structure(data, sects):
    ...
    with col2:
        ...
        st.caption("Nombre d'entreprises par tranche d'effectif salarie declare aupres de l'INSEE.")
    ...
    st.caption("Courbe de Pareto : les secteurs sont tries par volume decroissant. La ligne rouge indique le seuil des 80% du total.")
    ...
    st.caption("La nomenclature NAF classe les entreprises par activite (code a 2 chiffres). Ce graphique montre les 20 activites les plus frequentes parmi les entreprises actives.")
...
def page_survie(data):
    ...
    st.caption("Source : calcul INSEE sur les cohortes de creation. Ex : un taux a 3 ans de 65% signifie que 65 entreprises sur 100 creees la meme annee sont encore actives 3 ans apres.")
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("Survie selon l'annee de creation")
        st.caption("Les cases vides correspondent aux durees non encore observables pour les cohortes recentes.")
    ...
    with col2:
        ...
        st.caption("Comment s'erode le taux de survie au fil des annees, toutes cohortes confondues.")
    ...
    if not s3.empty:
        st.subheader("Le taux de survie a 3 ans s'ameliore-t-il dans le temps ?")
...
def page_carte(data, sects):
    ...
    with col2:
        ...
        st.caption("Chaque barre empilee montre la composition sectorielle du departement.")
...
def page_naf(data):
    section("NAF 2008 -> 2025", "La nomenclature d'activites francaise change le 1er janvier 2027 : quelles entreprises sont concernees ?")
    st.caption("La NAF classe les entreprises par type d'activite economique. La refonte de 2025 entraine des recodages. Ce tableau anticipe l'impact sur les entreprises actives aujourd'hui.")
    ...
    c2.metric("Code inchange", f"{ident}", f"{ident/total*100:.0f}% des divisions")
    c3.metric("Code recode", f"{recod}", f"{recod/total*100:.0f}% des divisions")
    c4.metric("Fusions (2->1)", fusio)
    ...
    with col2:
        ...
        fig2 = px.bar(di, x="nb_entreprises", y="lib_2008", orientation="h", color="type",
            color_discrete_map=COULEURS_NAF, labels={"lib_2008": "", "nb_entreprises": "Nb entreprises", "type": "Type"})
...
def main():
    ...
    st.sidebar.markdown(
        '<div style="background:#1565c0;padding:18px 16px 14px">'
        '<div style="color:#fff;font-size:1rem;font-weight:700">SIRENE Dashboard</div>'
...
def page_tendances(data, sects):
    ...
    fig.update_layout(title="Creations mensuelles (trait epais = moyenne mobile 3 mois)", hovermode="x unified",
        xaxis=dict(tickfont=dict(color="#374151")), yaxis=dict(tickfont=dict(color="#374151")))
    st.plotly_chart(fig, use_container_width=True)
    st.caption("Un pic en janvier est normal : beaucoup d'entrepreneurs immatriculent en debut d'annee.")
    st.divider()
    col1, col2 = st.columns(2)
    ...
    with col2:
        st.subheader("La dynamique s'accelere-t-elle ?")
        st.caption("Taux de croissance annuel. Attention : l'annee en cours est partielle.")
        ann = cm_f.groupby(["annee","grand_secteur"])["nb_creations"].sum().reset_index()
        ann = ann[ann["annee"] >= 2015].sort_values(["grand_secteur","annee"])
        ann["g"] = ann.groupby("grand_secteur")["nb_creations"].pct_change() * 100
        ann = ann.dropna(subset=["g"])
        fig3 = px.line(ann, x="annee", y="g", color="grand_secteur",
    ...
    pv = hg.pivot(index="grand_secteur", columns="annee", values="nb_creations").fillna(0)
    fig4 = px.imshow(pv, color_continuous_scale="Blues", labels={"color": "Nb creations"})
    fig4.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#ffffff",
...
def page_structure(data, sects):
    section("Structure du tissu economique", "Formes juridiques, taille, secteurs dominants")
    sa = data["stock_actives"]; fj = data["formes_juridiques"]; sa_f = sa[sa["grand_secteur"].isin(sects)]
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("Sous quelle forme juridique ?")
        fj_g = fj[fj["grand_secteur"].isin(sects)].groupby("forme_jur")["nb"]\
            .sum().reset_index().sort_values("nb", ascending=False)
        fig = px.pie(fj_g, values="nb", names="forme_jur", hole=0.45,
            color_discrete_sequence=["#1565c0","#42a5f5","#90caf9","#bbdefb","#e3f2fd","#dbeafe"])
    ...
    with col2:
        ...
        fig2 = px.treemap(tg, path=["libelle_taille"], values="nb",
            color="nb", color_continuous_scale="Blues")
        fig2.update_traces(texttemplate="<b>%{label}</b><br>%{value:,.0f}",
            textfont=dict(size=11, color="#374151"))
        fig2.update_layout(margin=dict(t=10,l=0,r=0,b=0), height=340,
            paper_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig2, use_container_width=True)
    st.divider()
    st.subheader("Quels secteurs concentrent le plus d'entreprises ?")
    st.caption("Pareto : tries par volume decroissant. La ligne rouge indique le seuil des 80% du total.")
    sg = sa_f.groupby("grand_secteur")["nb"].sum().reset_index().sort_values("nb", ascending=False)
    sg["cumul"] = sg["nb"].cumsum() / sg["nb"].sum() * 100
    fig3 = go.Figure()
    fig3.add_bar(x=sg["grand_secteur"], y=sg["nb"],
        marker_color=[COLOR_MAP.get(s,"#95a5a6") for s in sg["grand_secteur"]],
        name="Nb actives", yaxis="y")
    ...
    fig3.update_layout(
        plot_bgcolor="#ffffff", paper_bgcolor="rgba(0,0,0,0)", height=320,
        margin=dict(t=20,b=20,l=8,r=8),
        yaxis=dict(title="Nb entreprises", showgrid=False, tickfont=dict(color="#374151")),
        yaxis2=dict(title="Cumul %", overlaying="y", side="right",
            range=[0,110], showgrid=False, tickfont=dict(color="#374151")),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, font_color="#374151"),
        xaxis=dict(tickangle=15, showgrid=False, tickfont=dict(color="#374151")))
    st.plotly_chart(fig3, use_container_width=True)
    st.divider()
    st.subheader("Top 20 divisions NAF les plus representees")
    st.caption("La nomenclature NAF classe les entreprises par activite. Les 20 activites les plus frequentes parmi les entreprises actives.")
    top20 = data["naf_detail"][data["naf_detail"]["grand_secteur"].isin(sects)].nlargest(20, "nb")
    fig4 = px.bar(top20, x="nb", y="div_naf", orientation="h", color="grand_secteur",
        color_discrete_map=COLOR_MAP,
        labels={"div_naf": "Division NAF", "nb": "Nb actives", "grand_secteur": "Secteur"})
    T(fig4, 500)
    fig4.update_layout(yaxis=dict(showgrid=False, tickfont=dict(color="#374151")),
        xaxis=dict(tickfont=dict(color="#374151")))
    st.plotly_chart(fig4, use_container_width=True)


def page_survie(data):
    section("Survie des entreprises", "Quelle proportion reste active 1, 3 ou 5 ans apres la creation ?")
    df_s = data["survie"]
    if df_s.empty: st.warning("Donnees de survie non disponibles."); return
    st.caption("Ex : un taux a 3 ans de 65% signifie que 65 entreprises sur 100 creees la meme annee sont encore actives 3 ans apres. Source : calcul INSEE sur cohortes.")
    c1, c2, c3 = st.columns(3)
    for dur, col in [(1,c1),(3,c2),(5,c3)]:
        d = df_s[df_s["duree_ans"] == dur]
        if not d.empty:
            col.metric(f"Survie moyenne a {dur} an{'s' if dur>1 else ''}",
                f"{d['taux_survie'].mean():.1f}%")
    st.divider()
    st.subheader("Courbe de survie moyenne")
    st.caption("Comment s'erode le taux de survie au fil des annees, toutes cohortes confondues.")
    sa = df_s.groupby("duree_ans")["taux_survie"].mean().reset_index()
    fig2 = go.Figure()
    fig2.add_scatter(x=sa["duree_ans"], y=sa["taux_survie"],
        mode="lines+markers+text",
        text=sa["taux_survie"].map(lambda x: f"{x:.1f}%"),
    ...
    fig2.update_layout(
        ...
        xaxis=dict(title="Annees apres creation", showgrid=False,
            tickfont=dict(color="#374151")),
        yaxis=dict(title="% encore actives", range=[0,115],
    ...
    if not s3.empty:
        ...
        fig3.add_hline(y=s3["taux_survie"].mean(), line_dash="dash", line_color="#9ca3af",
            annotation_text=f"Moy. {s3['taux_survie'].mean():.1f}%")
        T(fig3, 280)
        fig3.update_layout(xaxis=dict(tickfont=dict(color="#374151")),
            yaxis=dict(tickfont=dict(color="#374151")))
        st.plotly_chart(fig3, use_container_width=True)
...
def page_carte(data, sects):
    ...
    col1, col2 = st.columns([2,1])
    with col1:
        mode = st.radio("Afficher",
            ["Nombre d'etablissements", "Densite sectorielle (%)"], horizontal=True)
    with col2:
        sf = st.selectbox("Focus secteur", ["Tous"] + sorted(sects))
    if "Nombre" in mode:
        md = dept_f.groupby("code_dept")["nb_etablissements"].sum().reset_index() \
            if sf == "Tous" else \
            dept_f[dept_f["grand_secteur"] == sf][["code_dept","nb_etablissements"]].copy()
        md.columns = ["code_dept","val"]; titre = "Etablissements"
    else:
        md = dept_f.sort_values("nb_etablissements", ascending=False)\
            .drop_duplicates("code_dept")[["code_dept","pct_secteur"]].copy() \
            if sf == "Tous" else \
            dept_f[dept_f["grand_secteur"] == sf][["code_dept","pct_secteur"]].copy()
        md.columns = ["code_dept","val"]; titre = "%"
    md["lat"] = md["code_dept"].map(lambda x: DEPT_COORDS.get(x,(None,None))[0])
    md["lon"] = md["code_dept"].map(lambda x: DEPT_COORDS.get(x,(None,None))[1])
    md["nom"] = md["code_dept"].map(DEPT_NOM).fillna(md["code_dept"])
    ...
    with col2:
        ...
        dt["nom"] = dt["code_dept"].map(DEPT_NOM).fillna(dt["code_dept"])
        dtg = dt.groupby(["nom","grand_secteur"])["nb_etablissements"].sum().reset_index()
        fig2 = px.bar(dtg, x="nom", y="nb_etablissements", color="grand_secteur",
            barmode="stack", color_discrete_map=COLOR_MAP,
            labels={"nom": "", "nb_etablissements": "Nb", "grand_secteur": "Secteur"})
        T(fig2, 320)
        fig2.update_layout(xaxis=dict(tickangle=40, showgrid=False,
            tickfont=dict(color="#374151")), yaxis=dict(tickfont=dict(color="#374151")))
        st.plotly_chart(fig2, use_container_width=True)


def page_naf(data):
    section("NAF 2008 -> 2025", "La nomenclature d'activites change le 1er janvier 2027 : quelles entreprises sont concernees ?")
    st.caption("La NAF classe les entreprises par activite economique. La refonte de 2025 entraine des recodages qui affecteront les series statistiques.")
    df_ch = pd.DataFrame(NAF_CHANGEMENTS,
        columns=["div_2008","lib_2008","div_2025","lib_2025","type"])
    naf_d = data["naf_detail"].copy()
    naf_d["div_naf"] = naf_d["div_naf"].astype(str).str.zfill(2)
    df_ch["div_2008"] = df_ch["div_2008"].astype(str).str.zfill(2)
    df_ch = df_ch.merge(
        naf_d.groupby("div_naf")["nb"].sum().reset_index()
            .rename(columns={"div_naf":"div_2008","nb":"nb_entreprises"}),
        on="div_2008", how="left").fillna({"nb_entreprises": 0})
    df_ch["nb_entreprises"] = df_ch["nb_entreprises"].astype(int)
    total = len(df_ch); ident = (df_ch["type"] == "Identique").sum()
    recod = (df_ch["type"] == "Recode").sum(); fusio = (df_ch["type"] == "Fusion").sum()
    nb_imp = df_ch[df_ch["type"] != "Identique"]["nb_entreprises"].sum()
    ...
    with col1:
        st.subheader("Combien d'entreprises par type de changement ?")
        dt = df_ch.groupby("type").agg(
            nb_div=("div_2008","count"), nb_ent=("nb_entreprises","sum")).reset_index()
        fig = px.treemap(dt, path=["type"], values="nb_ent", color="type",
            color_discrete_map=COULEURS_NAF, custom_data=["nb_div"])
        fig.update_traces(
            texttemplate="<b>%{label}</b><br>%{value:,.0f} entreprises<br>%{customdata[0]} divisions",
    ...
    with col2:
        st.subheader("Quelles divisions sont les plus impactees ?")
        di = df_ch[df_ch["type"] != "Identique"].nlargest(12, "nb_entreprises")
        fig2 = px.bar(di, x="nb_entreprises", y="lib_2008", orientation="h",
            color="type", color_discrete_map=COULEURS_NAF,
            labels={"lib_2008": "", "nb_entreprises": "Nb entreprises", "type": "Type"})
        T(fig2, 300)
        fig2.update_layout(yaxis=dict(showgrid=False, tickfont=dict(color="#374151")),
            xaxis=dict(tickfont=dict(color="#374151")))
        st.plotly_chart(fig2, use_container_width=True)
    st.divider()
    c1,c2,c3 = st.columns(3)
    with c1:
        ft = st.multiselect("Type", df_ch["type"].unique().tolist(),
            default=df_ch["type"].unique().tolist())
    with c2:
        rech = st.text_input("Rechercher", placeholder="ex: 62, informatique...")
    with c3:
        tri = st.selectbox("Trier par", ["Nb entreprises","Division 2008","Type"])
    df_a = df_ch[df_ch["type"].isin(ft)].copy()
    if rech:
        m = (df_a["div_2008"].str.contains(rech, case=False) |
             df_a["lib_2008"].str.contains(rech, case=False) |
             df_a["lib_2025"].str.contains(rech, case=False))
        df_a = df_a[m]
    if tri == "Nb entreprises": df_a = df_a.sort_values("nb_entreprises", ascending=False)
    elif tri == "Division 2008": df_a = df_a.sort_values("div_2008")
    else: df_a = df_a.sort_values("type")
    ...
    df_a.columns = ["Div.2008","Libelle 2008","Div.2025","Libelle 2025","Type","Nb entreprises"]
    st.dataframe(
        df_a.style.map(lambda v:
            f"color:{COULEURS_NAF.get(v,'#374151')};font-weight:600", subset=["Type"]),
        use_container_width=True, height=420, hide_index=True)
    st.caption(f"{len(df_a)} divisions affichees")


def main():
    st.set_page_config(page_title="Dashboard SIRENE", page_icon="",
    ...
    all_s = sorted(data["creations_mensuel"]["grand_secteur"].dropna().unique().tolist())
    sects = st.sidebar.multiselect("Secteurs", all_s,
        default=[s for s in all_s if s != "Autres"],
        label_visibility="collapsed")
    if not sects:
