"""
app.py - Dashboard SIRENE v2
Compatible avec prep_data v2 : employeurs.parquet restructure (4 categories)
"""
import warnings
...
COLOR_MAP = {"Agriculture":"#27ae60","Industrie":"#2980b9","Construction":"#d35400","Commerce / Transport / HCR":"#8e44ad","Services aux entreprises":"#c0392b","Services publics / Sante":"#16a085","Autres":"#7f8c8d"}
COLOR_EMP = {"Employeur":"#27ae60","Employeur occasionnel":"#f39c12","Non-employeur / NC":"#bdc3c7","Non renseigne":"#7f8c8d"}
...
def onglet_vue_ensemble(data):
    ...
    sa  = data["stock_actives"]
    cm  = data["creations_mensuel"]
    emp = data["employeurs"]
    total_actives = sa["nb"].sum()
    # Employeurs : depuis employeurs.parquet
    nb_emp = emp[emp["statut_employeur"]=="Employeur"]["nb"].sum()
    nb_ess = sa["nb_ess"].sum() if "nb_ess" in sa.columns else 0
    pct_emp = nb_emp/total_actives*100 if total_actives else 0
    pct_ess = nb_ess/total_actives*100 if total_actives else 0
    ...
    c3.metric("Dont employeurs", fmt(nb_emp), f"{pct_emp:.1f}%")
    c4.metric("Economie sociale (ESS)", fmt(nb_ess), f"{pct_ess:.1f}%")
    st.divider()
    ...
    st.subheader("Structure des entreprises par rapport a l-emploi")
    st.caption("Note : code NN = non-employeur OU absence de declaration (entreprises recentes <3 ans). La variable caractereEmployeurUniteLegale est abandonnee par l-INSEE depuis 2023.")
    emp_f = emp[emp["grand_secteur"].isin(data["creations_mensuel"]["grand_secteur"].dropna().unique())]
    ordre_emp = ["Employeur","Employeur occasionnel","Non-employeur / NC","Non renseigne"]
    fig3 = px.bar(emp_f, x="grand_secteur", y="nb", color="statut_employeur",
        barmode="stack", color_discrete_map=COLOR_EMP,
        category_orders={"statut_employeur": ordre_emp},
        title="Repartition par statut employeur",
        labels={"nb":"Nb","grand_secteur":"Secteur","statut_employeur":"Statut"})
...
def onglet_tendances(data, secteurs_choisis):
    ...
    cm = data["creations_mensuel"]
    cm_f = cm[cm["grand_secteur"].isin(secteurs_choisis)]
    ...
    for sect in d["grand_secteur"].unique():
        ...
        fig.add_scatter(x=ds["mois"], y=ds["nb_creations"], name=sect, mode="lines",
            line=dict(color=COLOR_MAP.get(sect,"#7f8c8d"), width=1), opacity=0.25, showlegend=False)
        fig.add_scatter(x=ds["mois"], y=ds["mm3"], name=sect, mode="lines",
            line=dict(color=COLOR_MAP.get(sect,"#7f8c8d"), width=2.5))
    fig.update_layout(title="Creations mensuelles par secteur (courbe = moyenne mobile 3 mois)",
        plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
        hovermode="x unified", legend=dict(orientation="h", yanchor="bottom", y=1.02),
        xaxis=dict(showgrid=False), yaxis=dict(showgrid=False),
        labels={"x":"Mois","y":"Creations"})
    st.plotly_chart(fig, use_container_width=True)
    st.divider()
    ...
    with col1:
        ...
        cm2["num_mois"] = pd.to_numeric(cm2["mois"].str[-2:], errors="coerce")
        ...
        fig2 = px.bar(sais, x="mois_nom", y="ecart",
            color=sais["ecart"].apply(lambda x: "Superieur" if x>=0 else "Inferieur"),
            color_discrete_map={"Superieur":"#27ae60","Inferieur":"#e74c3c"},
            title="Ecart a la moyenne mensuelle (%)",
            labels={"mois_nom":"Mois","ecart":"Ecart (%)"})
    ...
    with col2:
        ...
        ann = ann[ann["annee"].between(2015, ann["annee"].max())]
        ann_pct = ann.copy()
        ann_pct = ann_pct.sort_values(["grand_secteur","annee"])
        ann_pct["croissance"] = ann_pct.groupby("grand_secteur")["nb_creations"].pct_change() * 100
        ann_pct = ann_pct.dropna(subset=["croissance"])
        fig3 = px.line(ann_pct, x="annee", y="croissance", color="grand_secteur",
            color_discrete_map=COLOR_MAP, markers=True,
            title="Taux de croissance annuel des creations (%)",
            labels={"annee":"Annee","croissance":"Croissance (%)","grand_secteur":"Secteur"})
        fig3.add_hline(y=0, line_dash="dash", line_color="white", opacity=0.3)
        fig3.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
            hovermode="x unified", legend=dict(orientation="h", yanchor="bottom", y=1.02),
            xaxis=dict(showgrid=False), yaxis=dict(showgrid=False))
        st.plotly_chart(fig3, use_container_width=True)
    ...
    fig4 = px.imshow(heat_pivot, color_continuous_scale="Blues",
        title="Intensite des creations (plus c-est fonce = plus c-est eleve)",
        labels={"color":"Nb creations"})
...
def onglet_survie(data):
    st.header("Survie et longevite des entreprises")
    st.caption("Taux de survie = % d-entreprises encore actives apres N annees. Approximation via etat administratif actuel.")
    df_s = data["survie"]
...
def onglet_carte(data, secteurs_choisis):
    ...
    with col2:
        ...
        fig2 = px.bar(dt_g, x="nom", y="nb_etablissements", color="grand_secteur", barmode="stack",
            color_discrete_map=COLOR_MAP, labels={"nom":"Dept","nb_etablissements":"Nb","grand_secteur":"Secteur"})
        fig2.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
...
def onglet_naf(data):
    ...
    df_ch = df_ch.merge(naf_d.groupby("div_naf")["nb"].sum().reset_index().rename(columns={"div_naf":"div_2008","nb":"nb_entreprises"}),
    ...
    nb_tot=df_ch["nb_entreprises"].sum()
    ...
    with col1:
        ...
        fig = px.treemap(d_types, path=["type"], values="nb_ent", color="type",
            color_discrete_map=COULEURS_NAF, title="Entreprises concernees par type de changement",
            custom_data=["nb_div"])
    ...
    with col2:
        ...
        fig2.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
            height=350, xaxis=dict(showgrid=False))
        st.plotly_chart(fig2, use_container_width=True)
    st.divider()
...
def main():
    ...
    if data is None:
        ...
        st.info("Ce script telecharge les donnees SIRENE et genere les agregats. Duree : ~20 min.")
        return
    ...
    st.sidebar.divider()
    st.sidebar.caption("Source : INSEE / data.gouv.fr | MAJ mensuelle")
