...
app.py - Dashboard SIRENE v3
Navigation laterale, style epure Power BI, fond clair
"""
import warnings
from pathlib import Path
import pandas as pd
...
[data-testid="stSidebar"] .stRadio input[type="radio"], [data-testid="stSidebar"] [class*="radioMark"] { display:none !important; }
...
def ph(titre, sous=""):
    st.markdown(f'<div class="ph"><h2>{titre}</h2>'+(
        f'<p>{sous}</p>' if sous else '')+'</div>', unsafe_allow_html=True)
...
def page_vue_ensemble(data, sects):
    ...
    c2.metric("Creations 12 mois",fmt(c12m))
    c3.metric(f"Creations {ay}",fmt(an),f"{delta:+.1f}% vs {ay-1}")
    c4.metric("Employeurs",fmt(nb_emp),f"{nb_emp/total*100:.1f}%")
    c5.metric("ESS",fmt(nb_ess))
    st.divider()
...
def page_tendances(data, sects):
    ...
    for sect in sorted(d["grand_secteur"].unique()):
        ds=d[d["grand_secteur"]==sect]; col=COLOR_MAP.get(sect,"#95a5a6")
        fig.add_scatter(x=ds["mois"],y=ds["nb_creations"],name=sect,mode="lines",line=dict(color=col,width=1),opacity=0.18,showlegend=False)
        fig.add_scatter(x=ds["mois"],y=ds["mm3"],name=sect,mode="lines",line=dict(color=col,width=2.5))
    T(fig,360); fig.update_layout(title="Creations mensuelles - trait epais = moyenne mobile 3 mois",hovermode="x unified")
    st.plotly_chart(fig,use_container_width=True)
    st.divider()
    col1,col2=st.columns(2)
...
def page_structure(data, sects):
    ...
    st.subheader("Top 20 divisions NAF")
    top20=data["naf_detail"][data["naf_detail"]["grand_secteur"].isin(sects)].nlargest(20,"nb")
    fig4=px.bar(top20,x="nb",y="div_naf",orientation="h",color="grand_secteur",color_discrete_map=COLOR_MAP,labels={"div_naf":"Code NAF","nb":"Actives","grand_secteur":"Secteur"})
    T(fig4,480); fig4.update_layout(yaxis=dict(showgrid=False))
    st.plotly_chart(fig4,use_container_width=True)
